...
И таким образом можно регулировать работу очень многих функций в пандасе.



И так, далее groupby или в простонародье групбае. Идейно это разбиение таблицы на **группы** по какому-то признаку, потом применение операции к каждой группе и формирование таблицы с результатами.

К примеру у нас есть таблица. Экземпляры (строки) это разные цвета. Есть значение у каждой строки из столбца value. Мы можем сгруппировать по ЦВЕТУ, получаем 3 столбца с Value. На самом деле если бы у нас в первоначальной таблице был еще один столбец, допустим Transparency (прозрачность), то после groupby у нас бы было в каждом цвете по ДВА СТОЛБЦА. Один столбец с желтыми Value, второй столбец с желтыми Transparency.

```
df.groupby("Color")
```

Поэтому получив такое результат после применения метода groupby мы ОБРАЩАЕМСЯ к нужному нам столбцу. Допустим, к Value:

```
df.groupby("Color")["Value"]
```

Он выбирает для нас все три столбца Value для каждого цвета, прям как на рисунке:

<figure><img src="../.gitbook/assets/1bufYBdgnVTtd2ckDTooCRw.webp" alt="" width="563"><figcaption></figcaption></figure>

Но на этом моменте мы еще не можем никак выввести результат, он в виде объекта (можно проверить запустив код), чтобы получить результат нам нужно вызвать агрегирующую операцию:

```
df.groupby("Color")["Value"].mean()
```

Мы буквально применяем mean() (среднее) к каждому столбцу и получаем 3 разных средних для каждого цвета, из которых потом составляется финальная колонка, которую мы и получаем. Агрегирующие функции это по сути просто операция, применяемая к массиву данных. По типу найти среднее, стандартное отклонение, количество элементов, максимальное значение и все в таком духе. Основные функции: .sum(), .mean(), .count(). Да, я говорю функции, потому что на самом деле функция это более общее понятия, а метод более частное. Формально, выше это методы, но в самой начинке это те же функции, просто привязанные к чему-то, вот такие дела.

Таким образом, групбае нужно чтобы получить какую-то статистику по определенный столбцам. Мы могли бы брать группировку не по цвету, а по другому столбцу, тогда бы он группировал похожие значения из него, также и могли бы брать не Value, а другие столбец.
